...
void relax(Vertex *thisVertex, BinomialHeap *routeHeap){
    ...
    while(curr!=NULL){
        if(thisVertex->distanceFromStartVertex+curr->thisEdge->distance\
          <curr->thisEdge->dst->distanceFromStartVertex){
            curr->thisEdge->dst->distanceFromStartVertex = thisVertex->distanceFromStartVertex + curr->thisEdge->distance;
            ...
            //should also update the heapNode
            BinomialHeap *tmpHeap=newHeap();
            while(routeHeap->size!=0){
                HeapNode *currHeapNode=RemoveMin(routeHeap);
                InsertToHeap(tmpHeap,currHeapNode->thisVertex);
            }
            routeHeap=mergeHeaps(routeHeap,tmpHeap);
            printf("%d\n",routeHeap->size);
        }                                                                   
        curr=curr->next;
    }
...
void printRoute(Graph g,int srcStopId, int dstStopID){
    ...
    if(currV->distanceFromStartVertex==INF_DISTANCE){
        printf("No route is currently available, consider take a taxi?\n");
        return;
    }
    List *L=newList();
...
const char *BusNames,const char *BusRoutes,const char *Distance){
    ...
    BinomialHeap *routeHeap=newHeap();
    for(int id=0;id<g->nV;id+=1){
        InsertToHeap(routeHeap,g->vertices[id]);
    }
    while(routeHeap->size>0){
        Vertex *v=RemoveMin(routeHeap)->thisVertex;
        relax(v,routeHeap);
    }
